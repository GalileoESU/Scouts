...
// --- SPLIT SPREADSHEET INGESTION PROPRIETARY MECHANISMS ---

// 1. People Information Ingest
app.post('/api/manage/upload-people', upload.single('peopleFile'), async (req, res) => {
    try {
        ...
        const formatted = rows.map(r => ({
            group_number: String(r['Group Number'] || '').trim(),
            scout_group: String(r['Group Scout Group'] || '').trim(),
            category: String(r['Group Category'] || 'Scout').trim(),
            no_of_people: parseInt(r['No of People'] || 1),
...
// 2. Event Info Ingest (Waypoints / Checkpoints)
app.post('/api/manage/upload-event', upload.single('eventFile'), async (req, res) => {
    try {
        ...
        const formatted = rows.map(r => ({
            code: String(r['Checkpoint / Waypoint No'] || '').trim().padStart(3, '0'),
            type: String(r['Type'] || '').trim(),
            target_group: String(r['Group'] || 'All').trim()
...
app.post('/api/client/scan-waypoint', async (req, res) => {
    ...
    try {
        ...
        // Locate target configuration parameters
        ...
        // Category authorization gate matching spreadsheet rules
        ...
        // Duplication verify loop query check
        ...
        // Point value logic definition
        const valueReward = (team.category === 'Catcher') ? 10 : 10;
...
// Unified QR Scanner Catch & Check-in Processing Hook
app.post('/api/client/process-passport', async (req, res) => {
    ...
    try {
        ...
        // SCENARIO A: Request represents a physical operational location check-in action
        ...
        // SCENARIO B: Capture / Catch logging deployment processing rules
        if (operatorTeam && operatorTeam.category === 'Catcher') {
            ...
            if (graceRemainingMinutes > 0) {
                // Grace window block tracking log record trace
                await supabase.from('logs').insert([{
            ...
            // Execute standard deduction penalty rules
            await supabase.from('teams').update({
...
// --- BASIC AUTHENTICATION ENDPOINTS FOR FRONTEND LOGIN ---
